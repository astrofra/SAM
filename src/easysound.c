...
struct SoundInfo *PrepareSoundFromBuffer(char *buffer, int buffer_len, int replay_rate)
{
  ...
  if( info ) {
    info->FileLength = buffer_len;
    info->SoundBuffer = (BYTE *)buffer;
    info->RecordRate = replay_rate;
    printf("FileLength = %d, RecordRate = %d, SoundBuffer = %d.\n", info->FileLength, info->RecordRate, info->SoundBuffer);
